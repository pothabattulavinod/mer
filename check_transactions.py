...
json_url = "https://raw.githubusercontent.com/pothabattulavinod/rc/refs/heads/main/data.json"  # <-- replace with your raw GitHub URL
try:
...
def check_rc(rc_entry):
    ...
    # Check tables for current month
    has_transaction = any(
        (re.search(current_month, table.get_text(separator='\n', strip=True), re.IGNORECASE) or
         re.search(current_month[:3], table.get_text(separator='\n', strip=True), re.IGNORECASE)) 
